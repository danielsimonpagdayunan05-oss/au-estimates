...
export function ResetPasswordForm() {
  const { completePasswordReset, user } = useAdminAuth();
    ...
    <div className="mx-auto flex min-h-[70vh] max-w-md flex-col items-center justify-center px-5">
      ...
      <Card className="mt-8 w-full p-8">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h1 className="text-lg font-semibold text-ink-950 dark:text-white">Choose a new password</h1>
            <p className="text-xs text-ink-400">{user?.email ?? "Welcome back"}</p>
          </div>
        </div>
